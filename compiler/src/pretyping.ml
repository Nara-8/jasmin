...
let create_is_arr_init pd loc args =
  if List.length args == 3 then
    ...
    let _ = match t1 with
      | P.Arr _ -> ()
      | _ -> rs_tyerror ~loc (InvalidType (t1,TPArray)) 
    in
...
let rec tt_expr pd ?(mode=`AllVar) (env : 'asm Env.env) pe =
  ...
  | S.PEAbstract (id, args) ->
    ...
    if Env.is_abstract_pre env id then
      ...
      if Map.mem id init_predicates_map then
        let args = List.map (tt_expr ~mode pd env) args in
        let create_pred = Map.find id init_predicates_map in
        create_pred pd loc args
      else rs_tyerror ~loc (UnknownPred id)
