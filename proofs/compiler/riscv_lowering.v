...
Definition lower_Papp2
  ...
  let%opt _ := chk_ws_reg ws in
  match op with
  | Oadd (Op_k (Op_w _)) => decide_op_reg_imm U32 e0 e1 (BaseOp(None, ADD)) (BaseOp(None, ADDI))
  | Omul (Op_k (Op_w _)) => Some (BaseOp (None, MUL), [:: e0; e1])
  | Osub (Op_k (Op_w _)) => decide_op_reg_imm_neg U32 e0 e1 (BaseOp(None, SUB)) (BaseOp(None, ADDI))
  | Odiv (Cmp_w sg U32) =>
